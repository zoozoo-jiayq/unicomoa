					...
					if(data){
						...
						if(null!=data.images && ""!=data.images){
							...
							for(var i = 0 ; i < data.imagesList.length; i++){
								$("#imageAttachmentList").append("<li><div class=\"icon\"><em class=\""+data.imagesList[i]["attacthSuffix"]+"\"></em></div><div class=\"txt\"><p>"+data.imagesList[i]["attachName"]+"</p><p><a  style=\"cursor:pointer\"   onclick=\"deleteAttachment_daily("+data.imagesList[i]["id"]+",this);\">删除</a></p></li>");
							}
						}
						...
						if(null!=data.attment && ""!=data.attment){
							...
							for(var i = 0 ; i < data.attachmentList.length; i++){
								$("#attachmentList").append("<li><div class=\"icon\"><em class=\""+data.attachmentList[i]["attacthSuffix"]+"\"></em></div><div class=\"txt\" style=\"width:700px\"><p>"+data.attachmentList[i]["attachName"]+"</p><a  style=\"cursor:pointer\"  onclick=\"deleteAttachment_daily("+data.attachmentList[i]["id"]+",this);\">删除</a></p></li>");
							}
						}
...
//	var content = ue.getContent();
	...
	qytx.app.ajax({
				...
				shade:true,
				success : function(data) {
					window.location.href=basePath+"logined/notify/viewList.jsp?id="+$("#columnId").val();
				}
	});
}
