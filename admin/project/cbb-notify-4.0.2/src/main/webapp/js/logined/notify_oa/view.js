...
function subComment(){
	...
	if(comment==null || comment==""){
		qytx.app.dialog.alert("评论内容不能为空！");
		return ;
	}else if(comment.length>=500){
		qytx.app.dialog.alert("评论内容太长！");
		return ;
	}else{
		$.ajax({
			type : 'post',
...
//				if(returnType == 1){
					for (var i = 0; i < data.attachmentList.length; i++) {
						$("#attachmentList").append("<li><div class=\"icon\"><em class=\"doc\">" 
...
function transmit() {
	var notifyId = $("#notifyId").val();
	var columnId = $("#columnId").val();
	qytx.app.dialog.confirm('确定要转发该公告吗？', function() {
