	...
	public void getRedPointCount(){
		...
		try{
			...
			for(Enumeration  e = prop.propertyNames();e.hasMoreElements();){
				...
				try {
					if("notifyUnReadCount".equals(key)){
						columnId = 35;
					}else if("newsHospitalUnReadCount".equals(key)){
						columnId = 808;
					}else if("jobBriefUnReadCount".equals(key)){
						columnId = 814;
					}else if("superDocumentUnReadCount".equals(key)){
						columnId = 824;
					}else if("leaderSpeechUnReadCount".equals(key)){
						columnId = 830;
					}
