...
<!-- <script type="text/javascript" src="${ctx }/flat/js/smallTab.js"></script> -->
...
<input type="hidden" id="gongwenTypeFlag" value="1"/>
	    ...
	    <div class="operate">
	           <div class="tab">
	                <ul>
	                  ...
	                  <li><em class="swd"></em><a href="#">收文单</a></li>
	                  <li><em class="zw"></em><a href="#">正文</a></li>
	                  <li><em class="fj"></em><a href="#">附件</a></li>
	                   <li><em class="bs"></em><a href="#">版式文件</a></li>
	                </ul>
	          </div>
			  <div class="btn">
    ...
    <div class="content_form">
        ...
        <tr>
			...
			<td><input name="publicDomVo.sourceDept" id="dept"  value="" type="text" class="formText"valid="required" errmsg="gather_file.sourceDept_not_null" size="107" maxlength="50" /></td>
		</tr>		
	</table>
	<div class="con_explain">
      <ul>
        <li>注：文号自动生成，生成规则由管理员在系统设置中配置；</li>
      </ul>
    </div>
	</div>
	</div>
...
<script type="text/javascript">
	function windowOpen(url){
 	...
 	function getWenhao(domTypeId,dstInput){
		qytx.app.ajax({
			url:basePath+"dom/gather!getWenhao.action?documentTypeId="+domTypeId,
...
$("#next").click(function(){
	if(validator(document.getElementById("add"))){
		...
		var query = "publicDomVo.domTypeId="+domTypeId+"&publicDomVo.wenhao="+wenhao+"&publicDomVo.secretLevel="+secretLevel+"&publicDomVo.huanji="+huanji+"&publicDomVo.title="+title+"&publicDomVo.sourceDept="+dept+"&publicDomVo.domName="+domName+"&publicDomVo.domTypeName="+domTypeName+"&publicDomVo.firstLevel="+$("#firstLevel").val();
		qytx.app.ajax({
			url:basePath+"/dom/public!startDom.action?"+encodeURI(query)+"&r="+Math.random(),
			success:function(data){
				if(data){
					window.location.href=basePath+"dom/public!toMainByForward.action?taskId="+data+"&menu=1";
				}	
			}		
		});
	}
