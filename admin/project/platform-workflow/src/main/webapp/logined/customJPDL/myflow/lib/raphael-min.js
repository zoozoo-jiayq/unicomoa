 ...
 * 
 * Copyright (c) 2010 Dmitry Baranovskiy (http://raphaeljs.com) Licensed under
 * the MIT (http://raphaeljs.com/license.html) license.
 */
(function() {
	function a() {
	...
	var bi = function(b) {
		if (a.vml) {
			...
			bi = bm(function(a) {
						h.style.color = a;
						return g.defaultView.getComputedStyle(h, p)
	...
	a.getRGB = bm(function(b) {
				...
				if (k) {
					...
					k.hex = "#"
							+ (16777216 | g | e << 8 | d << 16).toString(16)
									.slice(1);
	...
	var bn = bm(function(a) {
		...
		var k = D * 120 / 180, l = D / 180 * (+e || 0), m = [], o, p = bm(
				function(a, b, c) {
		...
		if (j) {
			...
			h = o.x;
			i = o.y;
			var q = y.cos(D / 180 * e), r = y.sin(D / 180 * e), t = (a - h) / 2, u = (b - i)
					/ 2, x = t * t / (c * c) + u * u / (d * d);
			if (x > 1) {
				x = y.sqrt(x);
				c = x * c;
		...
		I = H - G;
		var M = y.cos(G), N = y.sin(G), O = y.cos(H), P = y.sin(H), Q = y.tan(I
				/ 4), R = 4 / 3 * c * Q, S = 4 / 3 * d * Q, T = [a, b], U = [
				a + R * N, b - S * M], V = [h + R * P, i - S * O], W = [h, i];
		U[0] = 2 * T[0] - U[0];
		U[1] = 2 * T[1] - U[1];
		{
			if (j)
	...
	if (a.svg) {
		...
		var bH = function(a, b) {
			...
			if (d == "linear") {
				...
				if (isNaN(i))
					return null;
				var j = [0, 0, y.cos(i * D / 180), y.sin(i * D / 180)], k = 1
						/ (z(B(j[2]), B(j[3])) || 1);
				j[2] *= k;
				j[3] *= k;
				if (j[2] < 0) {
			...
			for (var q = 0, t = m[w]; q < t; q++) {
				var u = bG("stop");
				bG(u, {
							offset : m[q].offset ? m[q].offset : q
									? "100%"
									: "0%",
			...
			var c = b.getBBox();
			bG(b.pattern, {
						patternTransform : a.format("translate({0},{1})", c.x,
								c.y)
					})
		}, bK = function(c, d) {
			var e = {
			...
			for (var n in d) {
				if (d[f](n)) {
					...
					switch (n) {
						...
						case "src" :
							c.type == "image"
									&& h.setAttributeNS(c.paper.xlink, "href",
											o);
							break;
						case "stroke-width" :
		...
		bN[e].attr = function(b, c) {
			...
			for (var k in this.paper.customAttributes)
				if (this.paper.customAttributes[f](k) && j[f](k)
					...
					var l = this.paper.customAttributes[k].apply(this,
							[][n](j[k]));
					this.attrs[k] = j[k];
					for (var m in l)
						l[f](m) && (j[m] = l[m])
	...
	if (a.vml) {
		var bW = {
			...
			c : "v",
			z : "x"
		}, bX = /([clmz]),?([^clmz]*)/gi, bY = / progid:\S+Blur\([^\)]+\)/g, bZ = /-?[^,\s-]+/g, b$ = 1000
				+ q + 1000, b_ = 10, ca = {
			path : 1,
			rect : 1
		}, cb = function(a) {
		...
		bK = function(c, d) {
			...
			if (d.opacity != null || d["stroke-width"] != null
				...
				d["stroke-linecap"]
						&& (C.endcap = d["stroke-linecap"] == "butt"
								? "flat"
		...
		bT = function(b, c, d, e) {
			...
			i.v = a.format("m{0},{1}l{2},{1}", Q(c * 10), Q(d * 10), Q(c * 10)
							+ 1);
			i.textpathok = true;
			h.width = b.width;
			h.height = b.height;
			...
			a[b] = bN[e][b] = function(c, d) {
				if (a.is(c, "function")) {
					...
					this.events.push({
								name : b,
								f : c,
								unbind : cj(this.shape || this.node || g, b, c,
										d || this)
							})
				}
				return this
			...
			if (this.type in {
				...
				if (this.transformations) {
					...
					this.node.filterMatrix = U
							+ ".Matrix(M11="[n](t, ", M12=0, M21=0, M22=", u,
					...
					if (n + j > e) {
						if (c && !l.start) {
							m = cq(g, h, i[1], i[2], i[3], i[4], i[5], i[6], e
											- n);
							k += ["C", m.start.x, m.start.y, m.m.x, m.m.y, m.x,
									m.y];
							if (f)
								return k;
							l.start = k;
