$(document).ready(function() {
    $("#file_upload").uploadify({
        ...
        //上传处理程序
        'uploader':basePath+'filemanager/uploadfile.action?module=documentType',
        //按钮文字
        'buttonText' : '上传附件...',
        //附带值
        'formData':{
        ...
        'onUploadSuccess':function(file, data, response){
        	data = eval('('+data+')');
        	attachId = data.id;
        	$("#imgAttachId").val(attachId);
        	var attachName = data.attachName;
            filePath = data;
            successUpload(attachId,attachName);
        },
...
function successUpload(attachId,attachName){
	$("#result").html("");
    $("<a>",{
        "text":attachName,
        "href":"#",
        "click":function(){
        	window.open(basePath+"filemanager/prevView.action?attachmentId="+attachId);
        }
    }).appendTo($("#result"));
    $("#result").append("&nbsp;&nbsp;");
    $("<img>",{
        "src":basePath+"/images/picDelete.png",
        "title":"删除",
        "click":function(){
            $("#result").html("");
            filePath = "";
