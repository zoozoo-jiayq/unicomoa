	        		...
	        		if(htmlType=="detail"){
	        			html+="<div class=\"details_show\">";
	        				...
	        				for(var j=0;j<details.length;j++){
	        					var detailMap=details[j];
	        					var index=j+1;
	        					html+="<span class=\"details_show_title\">";
		        				html+="<span>"+index+"</span>"+cnName;
		        				html+="</span>";
	        					for(var k=0;k<detailMap.arr.length;k++){
	        						var key=detailMap.arr[k].key;
	        						key=key.substring(0,key.lastIndexOf("_"));
	        						var value=detailMap.arr[k].value;
		        					html+="<table>";
					        		html+="<tr>";
					        		html+="<th>"+key+"：</th>";
					        		html+="<td>";
...
mui.plusReady(function(){
	...
	$("#title").html(title);
//	console.log(window.windowCommon.basePath+'workflow/detail!view.action?instanceId='+instanceId+'&userId='+window.windowCommon.approveLoginId+'&_clientType=wap')
	//查询表单数据
	mui.ajax(window.windowCommon.basePath+'workflow/detail!view.action?time='+(new Date()).getTime(),{
		...
		success:function(msg){
			...
			if(msg.indexOf("100||")==0){
            	...
            	var totalState=result.totalState;//0已结束 3审批中 1拒绝  2结束
            	var taskId=result.taskId;
            	var createUserPhoto=result.createUserPhoto;
            	var createUserName=result.createUserName;
            	$("#taskId").val(taskId);
            	if(createUserName&&createUserName.length>2){
						createUserName=createUserName.substring(createUserName.length-2,createUserName.length);
				}
            	var col=letterCode(createUserName);
            	$("#imgDiv").addClass("head-bg-"+col);
            	if(createUserPhoto&&createUserPhoto!="undefined"){
            		$("#photo").attr("src",window.windowCommon.photoUrl+createUserPhoto)
            	}else{
            		$("#photo").hide();
            		$("#imgName").html(createUserName);
            	}
            	//显示表单数据
	...
	rejectApply.addEventListener("release",function(){
		handle(1);
	});
	//同意审批
	var agreeApply=document.getElementById("agreeApply");
	agreeApply.addEventListener("release",function(){
		handle(0);
	});
	//撤销审批
	var revokeApply=document.getElementById("revokeApply");
	revokeApply.addEventListener("release",function(){
		handle(2);
	});
});
//返回
