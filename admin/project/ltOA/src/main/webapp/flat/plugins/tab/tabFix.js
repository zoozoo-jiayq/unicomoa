        ...
        if (mothodList != undefined && mothodList != "") {
           ...
           for (var i = 0; i < data.length; i++) {
                if (i == 0) {
                    ...
                    s_tabTxt += "<li  style='color: #dddddd'>|</li><li><a href=\"javascript:void(0);\" onclick=\"flushIframeUrl(this,'" + tabid + "','"+basePath + data[i].url + "')\">" + data[i].moduleName + "</a></li>";
                }
            }
            _tabArr[tabid]=1;
            showIndexHeader();
...
//       var window_w = $(window).width();/*当前屏幕的宽度*/
       ...
       if(window_w - tl_w < 80){
    	   ...
    	   $(".s_tab .lastMenu:visible").css("left",$("#div_tab li.crent").position().left + 90-($("#s_tab .lastMenu").last().width()-$("#div_tab li.crent").width())/2);
       }
      // 解决IE6 初次不加载问题
