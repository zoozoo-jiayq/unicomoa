...
function updateTypeOpen(vid,vname,typeValue,vorder) {
	...
	var id = vid;
    var typeName = vname;
	var url = basePath+'logined/dict/addDict.jsp?infoType='+infoType+"&typeName1="+vname+"&typeOrder="+vorder;
	art.dialog.open(url, {
	    id : "updateDict",
	    title : "修改考核指标",
	    width : 520,
...
function deleteSysType(){
        var SysTypeId=$("#SysTypeId").val();
        var infoType = $("#infoType").val();
		art.dialog.confirm('确定要删除所选分类吗？', function() {
				$.ajax({
