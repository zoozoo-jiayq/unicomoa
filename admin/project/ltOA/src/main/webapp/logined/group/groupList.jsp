			...
			<table class="pretty" cellpadding="0" cellspacing="0">
			    <thead>
					<tr>
						...
						<th  style="width:40%" class="right_bdr0" >职能</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${requestScope.groupTree}" var="group" varStatus="status">
                        <tr class='<c:if test="${status.index%2==1}">even</c:if><c:if test="${status.index%2==0}">old</c:if>'>
							...
							<td class="data_l right_bdr0" title='<c:out value="${group.functions}"/>'><c:out value="${group.functions}"/></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
