...
$(document).ready(function() {
	// 初始化部门树
	$("ul.service-menu li p").click(function(){ 
	     var cur= $(this).text();
	     if(cur=="个人知识库"){
	    	 openPerSelectTreeUser(perzTreeOnCheckResult, "", $("#personParentId").val());
	     }else if(cur=="公共知识库"){
	    	 openSelectTreeUser(zTreeOnCheckResult, "", $("#parentId").val());
	     }else if(cur=="我的收藏"){
	    	 openMySelectTreeUser(myzTreeOnCheckResult, "", $("#myparentId").val());
	     }
	});//一级菜单
...
function myzTreeOnCheckResult(data){
	var groupId = data.Id;
	tempGroupId = groupId;
	if (groupId != 0) {
		$("#page").attr("src", basePath + "group/loadGroupInfo.action?groupId=" + groupId);
...
function perzTreeOnCheckResult(data){
	var groupId = data.Id;
	tempGroupId = groupId;
	if (groupId != 0) {
		$("#page").attr("src", basePath + "group/loadGroupInfo.action?groupId=" + groupId);
