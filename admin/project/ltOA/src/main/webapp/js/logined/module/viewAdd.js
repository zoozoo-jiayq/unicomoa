...
function  subModule(){
	...
	if(nameValidate=="true"){
		qytx.app.valid.showObjError($("#name"), 'module.module_name_is_exit');
		return;
	}
	var code = $("#code").val();
...
//	});
	if(codeValidate=="true"){
		qytx.app.valid.showObjError($("#code"), 'module.module_code_is_exit');
		return;
	}
	var orderList = $("#orderList").val();
	if(orderList==null||orderList==""){
		qytx.app.valid.showObjError($("#orderList"), 'module.module_order_not_null');
		return ;
	}
	...
	if(orderListValidate==1){
		qytx.app.valid.showObjError($("#orderList"), 'module.module_order_is_exit');
		return;
	}else if(orderListValidate ==3){
		qytx.app.valid.showObjError($("#orderList"), 'module.module_parent_is_exit');
		return;
	}else{
		var orderList = $("#orderList").val();
		var url = $("#url").val();
		...
		var paramDate = {"mobile.name":name,"mobile.code":code,"mobile.url":url,"mobile.orderList":orderList,'mobile.icon':icon,"mobile.statue":statue};
		qytx.app.ajax({
			type:"post",
