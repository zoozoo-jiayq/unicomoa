...
function NTKO_OCX_OpenDoc(fileUrl)
{
	...
	setTimeout(function(){
	    ...
	    $('#li_review').addClass('current');
	    $('#li_revision_shwo').addClass('current');
	},100);
 ...
 * doc 转pdf
 */
function saveFileAsPdfToUrl(documentExtId)
{
	var myUrl = basePath+"servlet/NtkoPdfServlet"	;
	if(IsFileOpened)
	{
		OFFICE_CONTROL_OBJ.PublishAsPdfToURL(myUrl,"uploadPdf","documentExtId="+documentExtId,"","",true,false);
	}
}
...
function addEkeySecSign()
{
	if($("#signFlag").val()==1){
		if(IsFileOpened)
		{
		 ...
		 $("#formDiv").hide();
		art.dialog.alert("印章控件未启用！",function(){
			 $("#formDiv").show();
		});
	}
}
