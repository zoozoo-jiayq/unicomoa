$(document).ready(function() {
	...
	$("#addGroup").click(function() {
		window.location.href = basePath + "logined/address/add_group.jsp";
		return false;
	});
});
	        ...
	        .dataTable(
	                {
	                    ...
	                    "sAjaxSource" : basePath + "addressGroup/setup_getAddressGroupList.action",// 获取通讯薄联系人列表
	                    "sServerMethod" : "POST",
	                    ...
	                    "aoColumnDefs" : [ {
	                        ...
	                        "fnRender" : function(oObj) {
		                        ...
		                        // 编辑 删除 清空 导入 导出Foxmail格式 导出OutLook格式 打印
		                        var updateUrl = '<a href="' + basePath
		                                + "addressGroup/getDetailAddressGroup.action?id=" + id + '" >编辑</a>';
		                        var deleteUrl = '<a href="javascript:void();" onclick=deleteAddressGroup("' + id
		                                + '") >删除</a>';
...
function deleteAddressGroup(id) {
	art.dialog.confirm(sprintf("addressbook.msg_del_group_confirm"), function() {
		$.ajax({
		    url : basePath + "addressGroup/setup_deleteAddressGroup.action?deleteType=1&id=" + id,
		    type : "post",
		    dataType : "text",
		    success : function(data) {
			    if ("success" == data) {
				    ...
				    art.dialog.alert(sprintf("addressbook.msg_del_group_error"));
			    }
		    }
		});
	}, function() {
