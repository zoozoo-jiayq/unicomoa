...
public class UserListAction extends BaseActionSupport {
	...
	public String getUserListByGroupId(){
		...
		if (groupId!=null&&groupId!=0) {
			...
			List<GroupInfo> groups=groupService.getSubGroupList(groupId);
        ...
        if (userList != null) {
            for(int i=0; i<userList.size(); i++) {
                ...
                map.put("workNo", StringUtils.isNotBlank(user.getWorkNo())?user.getWorkNo():"--");
                map.put("officeTel", StringUtils.isNotBlank(user.getOfficeTel())?user.getOfficeTel():"--");
                if ("TXZL".equals(userVo.getProjectName())) {
                	  // 部门
                    String groupName = "";
